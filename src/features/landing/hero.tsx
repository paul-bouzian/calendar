		...
		<section className="relative py-20 md:py-32 px-6 overflow-hidden">
			...
			<div className="relative max-w-4xl mx-auto text-center">
				...
				<h1 className="text-4xl md:text-6xl lg:text-7xl font-bold tracking-tight mb-6">
					...
					<span className="text-foreground">{t("hero_title_rest")}</span>
				</h1>
				...
				<p className="text-lg text-muted-foreground/80 mb-10">
					<span className="font-medium text-foreground">
						{t("hero_example_phrase")}
					</span>{" "}
					{t("hero_example_result")}
				</p>
				...
				{/* Micro-copy */}
				<p className="text-sm text-muted-foreground">{t("hero_microcopy")}</p>

				{/* Social proof snippet */}
				<div className="mt-12 flex items-center justify-center gap-2 text-sm text-muted-foreground">
					<span className="inline-flex items-center gap-1.5">
						<span className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
						{t("hero_social_open_source")}
					</span>
					<span className="text-border">•</span>
					<span>{t("hero_social_join_users")}</span>
				</div>
			</div>
		</section>
	);
